...
void Router::route_one_datagram(InternetDatagram &dgram) {
    ...
    for (auto it = _routes.begin(); it != _routes.end(); ++it) {
        uint32_t mask = (it->length == 0) ? 0 : (0xFFFFFFFF << (32 - it->length));
        if ((dgram.header().dst & mask) == (it->prefix & mask)) {
    ...
    if (best_match != _routes.end()) {
        if (best_match->next_hop.has_value()) {
            ...
            _interfaces[best_match->interface_num].send_datagram(dgram, Address::from_ipv4_numeric(dgram.header().dst));
        }
    }
}
